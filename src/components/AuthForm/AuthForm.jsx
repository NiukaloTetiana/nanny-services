...
export const AuthForm = ({ registration, onClick }) => {
  ...
  const inputClass = (fieldName) => {
    const baseClass =
      "w-full bg-transparent border border-[#11101c19] rounded-[12px] px-[18px] py-[14px] lg:py-[16px] font-normal text-[14px] lg:text-[16px] leading-[1.25] text-[#11101c] placeholder:text-[#11101C] hover:shadow-custom-shadow focus:shadow-custom-shadow duration-300";
  ...
  const renderMessage = (fieldName) => {
    if (errors[fieldName]?.message && dirtyFields[fieldName]) {
      return (
        <p className="text-red-700 text-[10px] lg:text-[14px] font-normal absolute bottom-[-7px] left-[8px] px-[4px] bg-[#fbfbfb]">
          {errors[fieldName]?.message}
        </p>
      );
    }
    return (
      <p className="text-green-700 text-[10px] lg:text-[14px] font-normal absolute bottom-[-7px] left-[8px] px-[4px] bg-[#fbfbfb]">
  ...
  return (
    <>
      <h2 className="font-medium text-[30px] leading-[1.2] tracking-[-0.02em] text-[#11101c] mb-[20px] sm-max:text-[25px] sm-max:mb-[15px] lg:text-[40px]">
      ...
      >
          ...
          <div className="relative mb-[20px] sm-max:mb-[15px] lg:mb-[40px]">
            ...
            <button type="button" onClick={() => setShowPass((prev) => !prev)}>
              {showPass ? (
                <Icon
                  id="eye"
                  className="absolute top-1/2 right-4 transform -translate-y-1/2 stroke-[#11101c] fill-none lg:size-[20px]"
                  size="16"
                />
              ) : (
                <Icon
        ...
        <button
          type="submit"
          className="border-none rounded-[30px] px-[18px] py-[14px] lg:py-[16px] w-full bg-[#f03f3b] font-medium text-[14px] lg:text-[16px] leading-[1.25] tracking-[-0.01em] text-[#fbfbfb] hover:bg-[#D2110D] focus:bg-[#D2110D] duration-300"
        >

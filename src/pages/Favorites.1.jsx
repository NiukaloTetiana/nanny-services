...
const Favorites = () => {
    ...
    if (favoritesList.length) {
      ...
      getFavoritesNannies(favoritesList, page)
        .then((data) => {
          ...
          setIsLoadMore(data.length === 3); // Перевірка, чи є більше нянь для завантаження
        })
        .catch((error) => {
  ...
  const { filteredNannies, filterNannies } = useNanniesFilter(nannies); // Використовуємо хук для фільтрації
  ...
  return isLoading ? (
    ...
    <div className="bg-bgLigtColor min-h-screen">
      <div className="container pt-[45px] pb-[40px] md:pt-[64px] md:pb-[100px]">
        {nannies.length > 0 ? (
          <>
            <Filter filterFunction={filterNannies} />{" "}
            {/* Передаємо функцію фільтрації у компонент фільтра */}
            <NanniesList
              nannies={filteredNannies}
              handleClickLike={handleClickLike}
            />{" "}
            {/* Відображаємо відфільтровані няні */}
